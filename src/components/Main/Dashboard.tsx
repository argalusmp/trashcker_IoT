...
import { Scale, addScale, calculateTotalPrice, calculateTotalWeight } from "../../services/scale_db";
    ...
    <div className="relative p-4 w-full max-w-2xl max-h-full">
        <form className="bg-white relative rounded-lg shadow dark:bg-outline-color-theme" onSubmit={handleSubmit} method="POST">
            <div className="p-4 md:p-5 space-y-4 flex flex-col">
              ...
              <select className='dark:bg-black dark:text-white rounded-xl' onChange={(e)=> setUser(e.target.value)}>
                {users.map((user) => (
                  <option key={user.id} value={user.id}>{user.name}</option>
                ))}
              </select>
              <h3 className='dark:text-white font-semibold'>Jenis Sampah</h3>
              <select className='dark:bg-black dark:text-white rounded-xl' onChange={(e)=> setTrash(e.target.value)}>
                {trashes.map((trash) => (
            ...
            <div className='flex flex-col justify-center items-center mt-5'>
              ...
              <h1 className='text-white font-semibold text-3xl'>{ payload.weight ?? 0 }</h1>
            </div>
            <div className="flex items-center p-4 md:p-5 rounded-b mt-5">
                <button type="button" onClick={onSubmit} className="text-black bg-secondary-color-theme hover:bg-gray-200 font-semibold rounded-lg text-sm px-5 py-2.5 w-full text-center" data-modal-target="dashboard">Simpan</button>
            </div>
        </form>
    </div>
    </div>
