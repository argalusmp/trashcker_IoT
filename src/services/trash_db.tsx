...
async function index(): Promise<Trash[]>{
    ...
    res.docs.map((e) => {
        const data = e.data();
        scale.push(new Trash(data?.name, data?.code, data?.price, data?.created_at, data?.updated_at, e.id));
    })
    return scale;
}
...
export  {Trash, index as getTrashs, show as getTrashById, store as addTrash, update as updateTrash, remove as deleteTrash};